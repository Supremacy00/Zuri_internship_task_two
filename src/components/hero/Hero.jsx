  ...
  const renderSlides = heroData.map((item) => (
    <div key={item.id} className="text-white relative">
      ...
      <div className="max-w-[450px] text-left  mx-8 absolute top-[150px] md:top-[180px] xl:mx-28">
        <div>
          <h1 className="text-[24px] w-[280px] sm:text-[40px] md:w-[500px] sm:font-medium">
            {item.title}
          </h1>
          <div>
            <div className="flex items-center mt-2 gap-[50px]">
              ...
              <div className="flex gap-1.5 items-center mt-3">
                ...
                <p className="text-[13px]">{item.vote_count}</p>
              </div>
            </div>
          </div>
          <p className="mt-4 leading-7 sm:text-[18px] sm:mt-6">
            {item.overview}
          </p>
        </div>
